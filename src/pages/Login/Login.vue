            ...
            <section>
              ...
              <section class="login_verification">
                ...
                <div class="switch_button off" :class="showPwd?'on':'off'" @click="showPwd=!showPwd">
                  ...
                  <span class="switch_text">{{showPwd?'aaa':'...'}}</span>
                </div>
              </section>
              <section class="login_message">
...
<script>
  ...
  export default {
    data () {
      return {
        loginWay: false, //注册登录切换 true代表短信登陆, false代表密码
        computeTime: 0, // 计时的时间
        showPwd: false,//是否显示密码
        alertText: '',//弹出提示信息
        alertShow: false,//是否弹出
        phone: '',//手机号
        pwd: '',//密码
        name: '',//用户名
        code: '',//验证码
    ...
    methods: {
      ...
      async getCode () {
        if (this.computeTime <= 0) {
          this.computeTime = 30
          this.intervalId = setInterval(() => {//intervalId 清除定时器使用
            this.computeTime--
            if (this.computeTime <= 0) {
              // 停止计时
